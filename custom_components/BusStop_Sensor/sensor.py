...
DEFAULT_NAME = "Bus Stop Sensor(HK)"
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    stop_name = config.get(CONF_STOP_NAME)
    ...
    async_add_devices(
        [BusStopSensor(name, stop_name, routenumber, apikey, busdir)],
        update_before_add=True,
    )


class BusStopSensor(Entity):
    def __init__(self, name, stop_name, routenumber, apikey, busdir):
    ...
    def extra_state_attributes(self):
        ...
        attr["stop_name"] = self._stop_name
        ...
        if len(find1) == 3:
            Ltime1 = datetime.datetime.strptime(
                str(find1[0]["eta"][0:19]), "%Y-%m-%dT%H:%M:%S"
            )
            RLtime1 = parse(str(Ltime1))
            Ltime2 = datetime.datetime.strptime(
                str(find1[1]["eta"][0:19]), "%Y-%m-%dT%H:%M:%S"
            )
            Ltime3 = datetime.datetime.strptime(
                str(find1[2]["eta"][0:19]), "%Y-%m-%dT%H:%M:%S"
            )
    ...
    async def Start_Req_Data(self):
        """Start_Req_Data"""
        # _LOGGER.warning('Start_Req_Data')
        url = "http://data.etabus.gov.hk/v1/transport/kmb/eta/26AC2D471648CA0C/43A/1"
        # url="http://data.etabus.gov.hk/v1/transport/kmb/eta/" + self._stop_name + "/" + self._routenumber + "/1"
        self._name = self._stop_name + self._routenumber
        headers = "1"
        # _LOGGER.warning('url : ' + url)
        res_list = await self.asyncGet(url, headers)
        url1 = "https://data.etabus.gov.hk/v1/transport/kmb/stop"
        stop_list = await self.asyncGet(url1, headers)
        # _LOGGER.warning("res List " + json.dumps(res_list))
        # _LOGGER.warning("Stop List " + json.dumps(stop_list))
        json_data = stop_list
        find1 = jmespath.search("data[?name_tc==`" + self._stop_name + "`]", json_data)
        _LOGGER.warning("Stop List : " + str(len(find1)))
        return res_list

    @asyncio.coroutine
    async def async_update(self):
        # _LOGGER.warning('Async_update')
        res_list = await self.Start_Req_Data()
